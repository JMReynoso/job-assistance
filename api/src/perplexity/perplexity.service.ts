...
const RESEARCH_ANGLES: ResearchAngle[] = [
  ...
  },
  {
    label: 'news-hiring',
    focus:
      'recent news and current hiring signals (open roles, growth, layoffs)',
  },
];
...
export class PerplexityService {
  ...
  async research(
    ...
    const fulfilled = settled
      .filter(
        (s): s is PromiseFulfilledResult<AngleResult> => s.status === 'fulfilled',
      )
      .map((s) => s.value);
    ...
    return {
      reports: fulfilled.map((r) => ({ angle: r.angle, content: r.content })),
      sources,
    ...
    for (const result of results) {
      result.sources.forEach((src, index) => {
        ...
        if (existing) {
          ...
          bestPos.set(key, Math.min(bestPos.get(key) ?? index, index));
        } else {
  ...
  private aggregateUsage(results: AngleResult[]): CompanyResearchResult['usage'] {
   ...
   */
  private logUsage(model: string, usage: SonarUsage | undefined, angle: string): void {
    if (!usage) return;
    ...
    if (usage.reasoning_tokens) parts.push(`reasoning=${usage.reasoning_tokens}`);
    if (usage.citation_tokens) parts.push(`citation=${usage.citation_tokens}`);

    const cost = usage.cost?.total_cost;
    parts.push(cost !== undefined ? `cost=$${cost.toFixed(4)}` : 'cost=n/a');
