...
import type { JobStage, JobStageStatus } from "@/lib/job-assistance/types";
import { STAGE_META, activeStage, failedStage, isComplete, progressPercent } from "@/lib/job-assistance/job-progress";
import ConfirmCancelJobModal from "./ConfirmCancelJobModal";
...
export default function JobProgressModal({ companyName, stages, onCancel, onRetry, onDone }: JobProgressModalProps) {
  ...
  const statusLine = complete
    ? "All set — your tailored application is saved and ready to view."
    : failed
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-auto bg-[rgba(45,36,26,0.42)] px-5 py-10">
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="job-progress-modal-title"
      ...
      >
        <div className="flex items-start justify-between border-b border-table-border bg-[#fbf8f1] px-6 py-5">
          <div>
            <div id="job-progress-modal-title" className="font-heading text-[21px] font-semibold">
              {title}
            </div>
            <div className="mt-0.5 text-[13px] text-muted">
              {complete ? "Setup complete" : "Setting up your application…"}
            </div>
          </div>
          <button
            onClick={handleClose}
